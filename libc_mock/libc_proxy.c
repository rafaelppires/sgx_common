...
#ifdef TRACE_LIBC_CALLS
    printf(
        "FILE *freopen(const char *path='%s', const char *mode='%s', FILE "
        "*stream)\n",
        path, mode);
#endif
}

int fclose(FILE *f) { return fmock_close(f); }
...
int feof(FILE *f) { return fmock_feof(f); }
...
#ifdef TRACE_LIBC_CALLS
    if (ret == EOF) printf("int getc(FILE *f)\n");
#endif
    return ret;
}
...
size_t fread(void *ptr, size_t size, size_t nmemb, FILE *stream) {
    ...
    if (!ret)
        printf(
            "size_t fread(void *ptr, size_t size='%d', size_t nmemb='%d', FILE "
            "*stream='%ld')\n",
...
size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream) {
    ...
    if (stream == stdout || stream == stderr) {
        ...
        if (!ret)
            printf(
                "size_t fwrite(const void *ptr, size_t size='%d', size_t "
                "nmemb='%d', FILE *stream='%ld') : %d\n",
...
struct lconv *localeconv(void) {
    ...
    l.decimal_point = l.mon_decimal_point = ".";
