...
int initialize_enclave(sgx_enclave_id_t &global_eid,
                       const char *signed_so_fname, const char *token_fname) {
    ...
    if (fp != NULL) {
        ...
        if (read_num != 0 && read_num != sizeof(sgx_launch_token_t)) {
            ...
            printinfo(LLWARN,
                      "Warning: Invalid launch token read from \"%s\".\n",
                      token_path);
        }
    }
    ...
    ret = sgx_create_enclave(signed_so_fname, SGX_DEBUG_FLAG, &token, &updated,
                             &global_eid, NULL);
